...
from BetterBoardPlayer import BetterBoardPlayer
...
class PlayBoardGame:
    ...
    def select_starter(self):
        ...
        while self.to_move is None:
            type_of_computer = input("Choose option \n[1] Simple v Simple\n[2] Better v Simple \n ")
            if type_of_computer == "1":
                self.player1 = SimpleBoardPlayer(CROSS)
                self.player2 = SimpleBoardPlayer(RING)
                self.to_move = self.player1
                self.other = self.player2
            if type_of_computer == "2": 
                answer = input("Who starts? \n[1] Better\n[2] Simple \n ")
                if answer == "1":
                    self.player1 = BetterBoardPlayer(RING)
